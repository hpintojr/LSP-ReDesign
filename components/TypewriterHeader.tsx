...
const WORDS = [
  ...
  { text: 'consolidation options', highlight: 'from-af-red to-[#FF6B6B]' },
...
export default function TypewriterHeader() {
  ...
  return (
    <div id="hero-headline-container">
      <h1 className="font-display text-[41px] sm:text-5xl lg:text-[62px] font-extrabold tracking-[-0.03em] text-af-navy leading-[1.08] h-[90px] sm:h-auto sm:min-h-[110px] lg:min-h-[140px] overflow-hidden" id="typewriter-h1">
        Streamline your path to{' '}
        <br className="hidden lg:block" />
        <span className="inline-block relative">
          <span className={`bg-gradient-to-r ${currentHighlight} bg-clip-text text-transparent transition-all duration-300 drop-shadow-sm`} id="typed-text-span">
            {prefersReducedMotion ? WORDS[0].text : displayedText}
          </span>
          {!prefersReducedMotion && <span className="inline-block w-1 sm:w-1.5 h-[80%] bg-af-red align-middle ml-1 rounded-full animate-pulse" style={{ animationDuration: '0.85s' }} id="cursor-blink" />}
        </span>
      </h1>
    </div>
  );
