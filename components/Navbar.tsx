    ...
    <header id="site-header" className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-xl border-b border-af-blue-ice shadow-[0_4px_20px_-4px_rgba(15,23,42,0.06)] transition-all duration-200">
      <div className="bg-gradient-to-r from-af-navy-deep via-af-navy to-af-blue text-white py-1.5 px-4 text-xs">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 font-medium tracking-wide">
            ...
            <span className="hidden sm:inline text-white/90">A simpler way to explore options from independent Lending Partners</span>
            <span className="sm:hidden text-white/90 whitespace-nowrap">Explore Lending Partner options</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2 sm:justify-between sm:gap-0 h-[77px] sm:h-28">
          <Link href="/" onClick={() => analytics.navbarLogoClick()} className="flex items-center gap-1.5 group" aria-label="Loan Streamline Pro homepage">
            <div className="relative h-14 w-52 sm:h-24 sm:w-[330px] transition-transform duration-200 group-hover:scale-[1.02]">
              <Image src="/images/lsp-logo.svg" alt="Loan Streamline Pro — Smarter Connections, Simpler Options" fill className="object-contain object-left" priority sizes="(max-width: 768px) 208px, 330px" />
            </div>
          </Link>

          <div className="hidden lg:flex items-center gap-8">
            <nav className="flex items-center gap-2.5" aria-label="Main Navigation">
              <Link href="/" onClick={() => analytics.navbarLinkClick('home')} className="px-3.5 py-2 text-[17px] lg:text-[18px] font-bold text-af-navy hover:text-af-blue rounded-xl hover:bg-af-blue-soft transition-all duration-150">Home</Link>
              <Link href="/#loan-solutions-section" onClick={() => analytics.navbarLinkClick('services')} className="px-3.5 py-2 text-[17px] lg:text-[18px] font-bold text-af-navy hover:text-af-blue rounded-xl hover:bg-af-blue-soft transition-all duration-150">Services</Link>
              <Link href="/#estimator-anchor" onClick={(e) => { analytics.navbarLinkClick('apply_now'); handleOptionsClick(e); }} className="px-3.5 py-2 text-[17px] lg:text-[18px] font-bold text-af-navy hover:text-af-blue rounded-xl hover:bg-af-blue-soft transition-all duration-150">Apply Now</Link>
              <Link href="/blog" onClick={() => analytics.navbarLinkClick('blog')} className="px-3.5 py-2 text-[17px] lg:text-[18px] font-bold text-af-navy hover:text-af-blue rounded-xl hover:bg-af-blue-soft transition-all duration-150">Blog</Link>
            </nav>
        ...
        <div className="lg:hidden border-t border-af-blue-ice bg-white/98 backdrop-blur-2xl animate-fadeIn py-6 px-4 shadow-2xl" id="mobile-menu-drawer">
          <div className="flex flex-col gap-4">
            <nav className="flex flex-col gap-1.5">
              <Link href="/" className="px-4 py-3 text-lg font-bold text-af-navy hover:bg-af-blue-ice/60 rounded-xl transition-colors" onClick={() => { analytics.navbarLinkClick('home'); setIsOpen(false); }}>Home</Link>
              <Link href="/#loan-solutions-section" className="px-4 py-3 text-lg font-bold text-af-navy hover:bg-af-blue-ice/60 rounded-xl transition-colors" onClick={() => { analytics.navbarLinkClick('services'); setIsOpen(false); }}>Services</Link>
              <Link href="/#estimator-anchor" className="px-4 py-3 text-lg font-bold text-af-navy hover:bg-af-blue-ice/60 rounded-xl transition-colors" onClick={(e) => { analytics.navbarLinkClick('apply_now'); setIsOpen(false); handleOptionsClick(e); }}>Apply Now</Link>
              <Link href="/blog" className="px-4 py-3 text-lg font-bold text-af-navy hover:bg-af-blue-ice/60 rounded-xl transition-colors" onClick={() => { analytics.navbarLinkClick('blog'); setIsOpen(false); }}>Blog</Link>
            </nav>

            <div className="h-[1px] bg-af-blue-ice my-1" />
            <a href="tel:+18332890694" onClick={() => analytics.navbarCallClick('mobile')} className="flex items-center gap-4 p-4 rounded-2xl bg-af-blue-soft border border-af-blue-ice shadow-sm" id="mobile-menu-phone-call">
              <div className="flex items-center justify-center w-12 h-12 rounded-full bg-af-blue text-white shadow-md"><Phone className="w-6 h-6" /></div>
            ...
            <div className="text-center text-xs text-pv-muted px-4 mt-1">🔒 Technology Service • Independent Lending Partners • LSP Is Not a Lender</div>
          </div>
        </div>
      )}
    </header>
